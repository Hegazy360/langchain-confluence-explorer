...
def main():
    ...
    if st.button("Fetch Confluence Data", type="primary"):
        ...
        documents = loader.load(
            space_key="KB", include_attachments=True, limit=5, max_pages=10
        )
        for document in documents:
            ...
            st.session_state['chunks'].extend(text_splitter.split_text(document.page_content))

        if st.session_state['chunks']:
            embeddings = OpenAIEmbeddings()
            st.session_state['knowledge_base'] = FAISS.from_texts(st.session_state['chunks'], embeddings)

    if st.session_state['knowledge_base'] is not None:
        user_question = st.text_input("Ask a question about your Confluence space:")
